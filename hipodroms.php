...
$horses = [
    addHorse("Mustang", "M",1.3),
    addHorse("Reksis","R", 1.2),
    addHorse("Peksis","P", 1.6),
    addHorse("VW Golf","G",1.1),
    addHorse("T-34","T",2),
];
...
while($playAgain === "yes") {
    foreach ($horses as $key => $horse) {
        echo "$key. $horse->name     coefficient: $horse->coef " . PHP_EOL;
    }
...
///track display static
    ...
    while ($turn > 0) {
        ...
        foreach ($horses as $key => $horse) {
            ...
            if (isset($track[$previousPosition - 1])) {
                $track[$previousPosition - 1] = "_";
            }
            if (isset($track[$previousPosition - 2])) {
                $track[$previousPosition - 2] = "_";
            }
            if (isset($track[$previousPosition - 3])) {
    ...
    foreach ($horses as $key => $horse) {
        ...
        if ($myHorseKey === $key) {
            ...
            if ($myHorseName === $winners[0]) {
                ...
                echo $playAgain = readline("Play again?(yes/no)");
                echo PHP_EOL;
